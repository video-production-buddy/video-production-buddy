...
Walks through all 8 stages (research -> publish) with synthetic artifacts,
...
PROJECT_WORKSPACE = Path("projects") / "qa-e2e-test"
PROJECT_AUDIO_DIR = PROJECT_WORKSPACE / "assets" / "audio"
PROJECT_RENDER_DIR = PROJECT_WORKSPACE / "renders"


def _run_end_to_end_qa():
    global PASS, FAIL, final_video
    load_env()
    ...
    print("  Mixing audio...")
    mixer = AudioMixer()
    mix_output = str(PROJECT_AUDIO_DIR / "final_mix.wav")
    ...
    final_video = str(PROJECT_RENDER_DIR / "e2e_final_output.mp4")
    ...
    if os.path.exists(final_video):
        ...
        print("INSPECT: ffprobe summary above verifies container, streams, duration, and size; no GUI viewer was launched.")


def test_end_to_end_qa_script_succeeded():
    _run_end_to_end_qa()
    assert FAIL == 0, f"End-to-end QA reported {FAIL} failure(s)"
    assert os.path.exists(final_video), f"missing final video: {final_video}"
