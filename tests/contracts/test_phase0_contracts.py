...
from lib.config_model import VideoProductionBuddyConfig
        ...
        "decisions": [
            {
                ...
                "options_considered": [
                    ...
                    },
                    {
                        ...
                        "label": "Remotion",
                        "score": 0.5,
                        "reason": "Considered for React motion scenes.",
                        "rejected_because": "The fixture uses a simple ffmpeg runtime.",
                    },
                    {
                        "option_id": "hyperframes",
                        "label": "HyperFrames",
                        "score": 0.4,
                        "reason": "Considered for HTML/GSAP composition.",
                        "rejected_because": "The fixture does not require a HyperFrames workspace.",
                    },
                ],
                "selected": "ffmpeg",
...
class TestConfig:
    def test_load_defaults(self):
        config = VideoProductionBuddyConfig()
        assert config.llm.provider == "anthropic"
        assert config.budget.mode.value == "warn"
        assert config.checkpoint.policy.value == "guided"

    def test_load_from_yaml(self):
        config = VideoProductionBuddyConfig.load()
        assert config.budget.total_usd == 10.0

    def test_shared_dotenv_loader_accepts_export_assignments(self, tmp_path, monkeypatch):
        env_path = tmp_path / ".env"
        env_path.write_text(
            "export FAL_KEY=fal-from-env\n"
            "export QUOTED_SECRET='value # not a comment' # inline note\n",
            encoding="utf-8",
        )
        monkeypatch.delenv("FAL_KEY", raising=False)
        monkeypatch.delenv("QUOTED_SECRET", raising=False)
        monkeypatch.delenv("export FAL_KEY", raising=False)

        load_dotenv(env_path)

        assert os.environ["FAL_KEY"] == "fal-from-env"
...
class TestSchemas:
    ...
    def test_load_schema_rejects_path_like_artifact_names(self):
        with pytest.raises(ValueError, match="artifact schema name"):
            load_schema("../checkpoints/checkpoint")

    def test_validate_artifact_enforces_json_schema_formats(self):
        artifact = {
            "version": "1.0",
        ...
        with pytest.raises(jsonschema.ValidationError):
            validate_artifact("user_request", artifact)
...
""".lstrip()
    ...
    def test_get_completed_stages_ignores_other_pipeline_checkpoints(self, tmp_path):
        write_checkpoint(
            tmp_path,
            "proj",
            "script",
        ...
        completed = get_completed_stages(tmp_path, "proj", pipeline_type="ad-video")
    ...
    def test_ad_video_manifest_stage_accepted(self, tmp_path):
        ...
        cp = read_checkpoint(tmp_path, "proj", "intake", pipeline_type="ad-video")
        assert cp is not None
        assert cp["pipeline_type"] == "ad-video"
        assert cp["stage"] == "research.intake"

    def test_ad_video_legacy_stage_read_without_pipeline_hint(self, tmp_path):
        write_checkpoint(
            tmp_path,
            "proj",
            "intake",
...
class TestCostTracker:
    ...
    def test_reconcile_requires_reserved_entry(self):
        tracker = CostTracker(budget_total_usd=10.0, mode=BudgetMode.OBSERVE)
        entry_id = tracker.estimate("image_selector", "generate", 0.05)
    ...
    def test_completed_entry_cannot_be_reserved_again(self):
        tracker = CostTracker(budget_total_usd=10.0, mode=BudgetMode.OBSERVE)
        entry_id = tracker.estimate("image_selector", "generate", 0.05)
        tracker.reserve(entry_id)
        tracker.reconcile(entry_id, 0.04, success=True)
    ...
    def test_completed_entry_cannot_be_refunded(self):
        tracker = CostTracker(budget_total_usd=10.0, mode=BudgetMode.OBSERVE)
        entry_id = tracker.estimate("image_selector", "generate", 0.05)
        tracker.reserve(entry_id)
        tracker.reconcile(entry_id, 0.04, success=True)

        with pytest.raises(ValueError, match="completed.*refunded"):
...
""".lstrip()
    ...
    def test_paid_tool_approval_persists(self, tmp_path):
        ...
        t1.approve_tool("image_api")
        eid = t1.estimate("image_api", "generate", 0.10)
        t1.reserve(eid)
        ...
        resumed_eid = t2.estimate("image_api", "generate", 0.10)
        t2.reserve(resumed_eid)

        assert t2.budget_reserved_usd == 0.20

    def test_single_action_approval_allows_reserve_retry(self):
        tracker = CostTracker(
            budget_total_usd=10.0,
            mode=BudgetMode.WARN,
            single_action_approval_usd=0.50,
        )
        tracker.approve_tool("image_api")
        entry_id = tracker.estimate("image_api", "generate", 1.00)

        with pytest.raises(ApprovalRequiredError, match="single-action threshold"):
            tracker.reserve(entry_id)

        assert tracker.budget_reserved_usd == 0.0

        tracker.approve_action(entry_id)
        tracker.reserve(entry_id)

        assert tracker.budget_reserved_usd == 1.00
