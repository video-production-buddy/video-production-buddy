...
from lib.pipeline_loader import get_required_tools, get_stage_definition, load_pipeline
...
def test_ad_video_manifest_and_skills_require_professional_knowledge_retrieval() -> None:
    ...
    intelligence_stage = get_stage_definition(manifest, "research.intelligence")
...
def test_ad_video_manifest_declares_dynamic_genui_for_human_gates() -> None:
    """Human gates must expose the dynamic router plus v4 session plumbing."""
    ...
    for stage_name in [
        ...
        stage = get_stage_definition(manifest, stage_name)
        assert stage is not None, stage_name
        assert "genui_interaction" in _stage_tool_names(stage), stage_name
        assert "genui_session" in _stage_tool_names(stage), stage_name

    stages = {stage["name"]: stage for stage in manifest["stages"]}
    asset_substages = {
        substage["name"]: substage for substage in stages["assets"].get("sub_stages", [])
    }
    for substage_name in ["product_reference", "sample", "asset_review", "music_review"]:
        assert "genui_interaction" in asset_substages[substage_name].get("tools_available", []), substage_name
        assert "genui_session" in asset_substages[substage_name].get("tools_available", []), substage_name

    assert "genui_interaction" in get_required_tools(manifest)
    assert "genui_session" in get_required_tools(manifest)
    assert "genui_surface" not in get_required_tools(manifest)


def test_ad_video_visual_gates_require_genui_v7_sessions_not_cli_only() -> None:
    """Visual/multi-axis gates must instruct agents to use dynamic GenUI v7 by default."""
    cases = {
        "idea-director.md": [
            "conceptcomparison",
            "selected_concept_id",
    ...
    for filename, required_phrases in cases.items():
        text = " ".join(_read_skill(filename).lower().replace("`", "").split())
        assert "genui v7" in text, filename
        assert "genui_interaction" in text, filename
        assert "ui_session_config" in text, filename
        assert "ui_session_response" in text, filename
        assert "approvalchecklist" in text, filename
        assert "genui_session execution fails" in text, filename
        assert "explicitly declines the browser path" in text, filename
        for phrase in required_phrases:
            assert phrase in text, f"{filename} missing {phrase!r}"


def test_ad_video_review_gates_do_not_default_to_manual_file_opening() -> None:
    """Reviewable media should be shown inline in GenUI unless the browser path fails."""
    text = " ".join(_read_skill("asset-director.md").lower().replace("`", "").split())
...
def test_ad_video_contract_mentions_trend_alignment_flow() -> None:
    ...
    intelligence_stage = get_stage_definition(manifest, "research.intelligence")
    bible_stage = get_stage_definition(manifest, "proposal.bible")
    script_stage = get_stage_definition(manifest, "script")
    scene_stage = get_stage_definition(manifest, "scene_plan")
    contract_text = "\n".join(
        intelligence_stage.get("review_focus", [])
...
def test_ad_video_manifest_brief_enrichment_review_focus_checks_creative_requirements() -> None:
    ...
    manifest = _load_ad_video_manifest()
    brief_enrichment_stage = get_stage_definition(manifest, "research.brief_enrichment")
...
def test_ad_video_manifest_proposal_success_criteria_use_proposal_locks() -> None:
    ...
    manifest = _load_ad_video_manifest()
    proposal_stage = get_stage_definition(manifest, "proposal.technical_proposal")
...
def test_ad_video_technical_proposal_director_teaches_confirmed_subtitle_lock() -> None:
    """Technical proposal guidance must match schema's explicit subtitle confirmation gate."""
    manifest = _load_ad_video_manifest()
    proposal_stage = get_stage_definition(manifest, "proposal.technical_proposal")
    success_text = "\n".join(proposal_stage.get("success_criteria", []))
    proposal = _read_skill("technical-proposal-director.md")
...
def test_ad_video_contract_mentions_product_identity_reference_flow() -> None:
    ...
    proposal_stage = get_stage_definition(manifest, "proposal.technical_proposal")
    scene_stage = get_stage_definition(manifest, "scene_plan")
    asset_stage = get_stage_definition(manifest, "assets")
...
def test_screenshot_scene_registry_matches_composer_contract() -> None:
    registry = _load_scene_type_registry()
    scene_types = registry["scene_types"]

    screenshot = scene_types["screenshot_scene"]
    assert screenshot["required_props"] == ["backgroundImage", "screenshotSteps"]
    assert "screenshotSize" in screenshot["optional_props"]
    assert "cursorStartAt" in screenshot["optional_props"]
    assert "synthetic ui" in screenshot["description"].lower()
    assert {
        "cursor_move",
        "click_pulse",
        "type_into",
        ...
        "highlight_box",
        "callout_balloon",
    } <= set(screenshot["motion_primitives"])
    assert "screenshot_scene" in VideoCompose._REMOTION_COMPONENTS


def test_scene_type_registry_indexes_every_declared_motion_primitive() -> None:
    """Bible-stage KVMs must only reference primitives the registry defines."""
    registry = _load_scene_type_registry()
    indexed = set(registry["motion_primitives_index"])
    declared = {
        primitive
        for scene_type in registry["scene_types"].values()
        for primitive in scene_type.get("motion_primitives", [])
    }

    assert declared <= indexed


def test_remotion_scene_types_cheat_sheet_lists_every_registered_type() -> None:
    """SCENE_TYPES.md is the human-facing companion to the registry."""
    registry = _load_scene_type_registry()
    cheat_sheet = (ROOT / "remotion-composer" / "SCENE_TYPES.md").read_text()
    missing = [
        scene_type
        for scene_type in registry["scene_types"]
        if f"`{scene_type}`" not in cheat_sheet
    ]
...
def test_ad_video_contract_mentions_hallucination_flow() -> None:
    ...
    scene_stage = get_stage_definition(manifest, "scene_plan")
    asset_stage = get_stage_definition(manifest, "assets")
    compose_stage = get_stage_definition(manifest, "compose")
    publish_stage = get_stage_definition(manifest, "publish")
    contract_text = "\n".join(
