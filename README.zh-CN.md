...
HyperFrames doctor 打印 Docker 警告不一定影响本地渲染；关键是命令成功退出，并报告 runtime available。
...
这条 demo 路径使用本地 Remotion 组件，不需要云端 API key。它会渲染多个演示视频，普通笔记本上需要几分钟。生成的视频位于 `projects/demos/renders/`。
