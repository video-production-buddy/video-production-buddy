...
def _is_json_schema_datetime(value: object) -> bool:
...
def _is_json_schema_uri(value: object) -> bool:
    if not isinstance(value, str):
        return True
...
def load_schema(name: str) -> dict:
    ...
    if (
        ...
        raise ValueError(
            f"Invalid artifact schema name {name!r}; expected a bare artifact schema name"
        )
...
def _validate_intelligence_brief(
    ...
    if pipeline_type == "ad-video":
        for idx, verdict_entry in enumerate(data.get("dimension_verdicts") or []):
            if not isinstance(verdict_entry, dict):
                continue
            confidence = verdict_entry.get("confidence")
            verdict = verdict_entry.get("verdict")
            if confidence == "default-heuristic" and verdict != "INSUFFICIENT-DATA":
                raise jsonschema.ValidationError(
                    "ad-video intelligence_brief.dimension_verdicts"
                    f"[{idx}] with confidence='default-heuristic' must use "
                    "verdict='INSUFFICIENT-DATA'"
                )
            if confidence == "research-grounded" and verdict == "CONTRADICTED":
                challenge_evidence = verdict_entry.get("challenge_evidence")
                if (
                    not isinstance(challenge_evidence, str)
                    or not challenge_evidence.strip()
                ):
                    raise jsonschema.ValidationError(
                        "ad-video intelligence_brief.dimension_verdicts"
...
def _validate_ad_video_render_report(
    ...
    unrendered_probe_variants = sorted(probe_variants - rendered_variants)
    if unrendered_probe_variants:
        raise jsonschema.ValidationError(
            "ad-video render_report.probe_results contains unrendered "
            "variant(s): "
            + ", ".join(repr(variant) for variant in unrendered_probe_variants)
        )
...
def _validate_decision_log(data: dict[str, Any]) -> None:
    ...
    for idx, decision in enumerate(data.get("decisions", []) or []):
        ...
        if decision.get("category") == "render_runtime_selection":
            missing_runtime_options = {"remotion", "hyperframes"} - option_ids
            if missing_runtime_options:
                raise jsonschema.ValidationError(
                    "decision_log.decisions"
                    f"[{idx}].options_considered for render_runtime_selection "
                    "must include remotion and hyperframes option_id entries, "
                    "including rejected/unavailable options"
                )
