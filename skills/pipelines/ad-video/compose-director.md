...
production_proposal = load_json("projects/<name>/artifacts/production_proposal.json")
...
result = composer.execute({
    ...
    "output_path": "projects/<name>/renders/output_16x9.mp4",
})
if result.error:
    raise RuntimeError(f"video_compose primary render failed: {result.error}")
```

**HyperFrames note:** when `edit_decisions.render_runtime == "hyperframes"`, `video_compose` routes internally to the HyperFrames engine (HTML/GSAP motion primitives). Do NOT attempt to construct a separate HyperFrames invocation — carry `render_runtime="hyperframes"` in `edit_decisions` and let the tool route correctly.
