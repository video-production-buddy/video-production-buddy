...
EP_STATE:
  pipeline: ad-video
  style_mode: null            # locked at proposal.technical_proposal: "animated" | "cinematic"
  render_runtime: null        # locked at proposal.technical_proposal: "remotion" | "hyperframes" | "ffmpeg"
  product_reference_strategy: null # locked at proposal.technical_proposal: not_applicable | use_provided_reference | generate_concept_reference | risk_accepted
  playbook: ad-brand
  ...
  aspect_ratio_primary: "16:9" # locked at proposal.technical_proposal: "16:9" | "9:16" — determines primary render resolution and scene framing
  derivative_variants: []     # locked at proposal.technical_proposal: subset of ["9:16", "1:1", "15s"]
  sample_approved: false      # set true after sample sub-stage approval

  artifacts:
    research.intake: null       # → intake_brief
    research.brief_enrichment: null   # → enriched_brief
    research.intelligence: null # → intelligence_brief
    proposal.bible: null        # → production_bible (includes approval flags)
    proposal.idea: null
    proposal.technical_proposal: null
...
3. Set budget: default $5.00, override from `production_proposal.approved_budget_usd` after `proposal.technical_proposal`
4. Initialize EP_STATE

### Phase 1: Execute Checkpointable Gates Serially

Top-level order: `research → proposal → script → scene_plan → assets → edit → compose → publish`

Checkpointable gate order: `research.intake → research.brief_enrichment → research.intelligence → proposal.bible → proposal.idea → proposal.technical_proposal → script → scene_plan → assets → edit → compose → publish`

**Pre-production gates (`research.*`, `proposal.*`, `script`, `scene_plan`)** — zero or near-zero cost, no generative tool calls until assets stage.

After `proposal.bible` approval (Round 2a + 2b), extract and store in EP_STATE:
...
After `proposal.technical_proposal` approval, extract and store in EP_STATE:
...
EXECUTE_STAGE(stage_name):
  1. PREPARE
     Load director skill for this stage or child gate.
...
|------|------------|----------------|-------------|
| G-0 | research.brief_enrichment | Three blockers resolved; creative_requirements complete; user_approved=true | Wait for user |
| G1 | proposal.idea | Brief completeness, style_mode_candidate, ref role annotations | Revise idea |
